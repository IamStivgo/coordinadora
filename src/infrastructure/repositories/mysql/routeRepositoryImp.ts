...
export class RouteRepositoryImp implements IRouteRepository{
    ...
    async create(route: CreateRouteDTO): Promise<RouteEntity | null> {
        ...
        const params = [id, name, origin, destination, distance, departureTime, departureDate];
    ...
    async update(route: RouteEntity): Promise<RouteEntity> {
        ...
        const params = [name, origin, destination, distance, departureTime, departureDate, arrivalTime, arrivalDate, driverId, vehicleId, createdAt, updatedAt, id];
